...
function mount_other_volumes() {
    ...
    local audio_image="${DOCKER_REPO}:data_volume-audio_model-${TARGET_ARCH}-${VOLUME_VERSION}"
    ...
    local yolov4_image="${DOCKER_REPO}:yolov4_volume-emergency_detection_model-${TARGET_ARCH}-${VOLUME_VERSION}"
    ...
    local faster_rcnn_image="${DOCKER_REPO}:faster_rcnn_volume-traffic_light_detection_model-${TARGET_ARCH}-${VOLUME_VERSION}"
    ...
    if [[ "${TARGET_ARCH}" == "x86_64" ]]; then
        ...
        local smoke_image="apolloauto/apollo:smoke_volume-yolo_obstacle_detection_model-${TARGET_ARCH}-latest"
